...
class Player():
    ...
    def take_action(self, highest_bet, limit, state, blind=None):
        ...
        if blind == 'small':
            ...
            # Change: work out what happens if player runs out of money.
            if highest_bet == self.stack_original:
                ...
                print(f'Player {self.name}: I have',
                      f'{self.actions[self.last_actions[-1]]}ed and am betting ${self.own_bet}!')
        else:
            # Change: make sure that only 1, 2, 3 can be entered.
            txt = input("Please chose your action (fold (1), check or call(2), raise(3)): ")
            action = int(txt)
            if action == 1:
                self.fold()
            elif action == 2:
...
class Game:
    ...
    def determine_order(self):
        ...
        if self.game_count != 1:
            ...
            # Test: self.order[nr_of_players-1] before a game is
            # self.order[nr_of_players-2] during the game.
    ...
    def collect_data(self, player, position):
        ...
        # Change: once we introduce playing with different sized stacks the next
        # line will cause problems
        data['bet'] = self.players[player].own_bet/self.stack
        ...
        for player_number in self.order:
            if player_number != player:
                ...
                data[f'bet_{count}'] = self.players[player_number].own_bet/self.stack
        ...
        while not check_activity:
            ...
            for position, player in enumerate(self.order):
                ...
                and self.players[player].active == 1:
                    ...
                    # Change: has to be modelled correctly so that players can
                    # only play when they have the money
    ...
    def action_first_round(self):
        ...
        self.players[self.order[0]].take_action(self.highest_bet, self.limit, [], blind='small')

        # action of the big blind player
        self.players[self.order[1]].take_action(self.highest_bet, self.limit, [], blind='big')
